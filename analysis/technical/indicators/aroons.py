...
class AROONS(TechnicalIndicator):
    ...
    def _compute_indicator_values(self, highs: pd.Series, lows: pd.Series, window: int = 25) -> tuple[pd.Series, pd.Series]:
        aroon_values = AroonIndicator(high=highs, low=lows, window=window)
